...
import { CheckCertificationResponsetDto, EmailCertificationResponseDto, IdCheckResponseDto, SignInResponseDto, SignUpResponseDto } from "./response/auth";
...
export const SNS_SIGN_IN_URL = (type: 'kakao' | 'naver'): string => `${API_DOMAIN}/auth/oauth2/${type}`;
...
export const CheckCertificationRequest = async (requestBody: CheckCertificationRequestDto) => {
    const result = await axios.post(CHECK_CERTIFICATION_URL(), requestBody)
        .then(responseHandler<CheckCertificationResponsetDto>)
        .catch(errorHandler)
    return result;
};